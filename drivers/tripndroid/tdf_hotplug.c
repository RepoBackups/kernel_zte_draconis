...
static struct notifier_block tdf_lcd_notif;
...
static void __ref tdf_cpu_up(int cpu) {
    if (!cpu_online(cpu)) {
...
static void tripndroid_hp_early_suspend(struct notifier_block *handler)
{
	int i = nr_cpu_ids;
...
static void tripndroid_hp_late_resume(struct notifier_block *handler)
{
	int i = nr_cpu_ids;
	int max_cpus;
...
static int tdf_lcd_notifier_callback(struct notifier_block *this,
				unsigned long event, void *data) {
	pr_debug("%s: event = %lu\n", __func__, event);

	switch (event) {
	case LCD_EVENT_ON_START:
		tripndroid_hp_late_resume(this);
		break;
	...
	case LCD_EVENT_OFF_END:
		tripndroid_hp_early_suspend(this);
		break;
	default:
		break;
	}
...
static int __init tripndroid_hp_init(void)
{
	...
	tdf_lcd_notif.notifier_call = tdf_lcd_notifier_callback;
	if (lcd_register_client(&tdf_lcd_notif) != 0) {
		pr_err("%s: Failed to register lcd callback\n", __func__);
		err = -EINVAL;
		lcd_unregister_client(&tdf_lcd_notif);
	}
